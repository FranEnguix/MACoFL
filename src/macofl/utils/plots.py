...
class LogData:
    ...
    def load_data(self):
        ...
        if self.algorithm_rounds is not None:
            self.data = self.data[
                self.data["algorithm_round"].isin(self.algorithm_rounds)
            ]
...
class AlgorithmData(LogData):
    ...
    def process_data(self):
        ...
        self.avg_time_per_agent = (
            self.data.groupby("agent")["seconds_to_complete"].mean().reset_index()
        )
        self.avg_time_per_round = (
            self.data.groupby("algorithm_round")["seconds_to_complete"]
            .mean()
            .reset_index()
        )
...
class MessageData(LogData):
    ...
    def process_data(self):
        ...
        self.sent_messages_per_round = (
            self.data[self.data["is_sent"]]
            .groupby("algorithm_round")
            .size()
            .reset_index(name="sent_message_count")
        )
        self.message_counts_per_sender = (
            self.data.groupby("sender").size().reset_index(name="counts")
        )
        self.message_counts_per_type = (
            self.data.groupby("type").size().reset_index(name="counts")
        )
...
class NNInferenceData(LogData):
    ...
    def process_data(self):
        ...
        self.avg_test_accuracy_per_agent = (
            self.data.groupby("agent")["test_accuracy"].mean().reset_index()
        )
        self.avg_test_loss_per_agent = (
            self.data.groupby("agent")["test_loss"].mean().reset_index()
        )
...
class NNTrainData(LogData):
    ...
    def process_data(self):
        ...
        self.avg_accuracy_per_agent = (
            self.data.groupby("agent")["accuracy"].mean().reset_index()
        )
        self.avg_loss_per_agent = (
            self.data.groupby("agent")["loss"].mean().reset_index()
        )
