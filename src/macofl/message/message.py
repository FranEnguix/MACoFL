...
class RfMessage(Message):
    ...
    @staticmethod
    def from_message(
        message: Message, is_multipart: bool, is_multipart_completed: bool
    ) -> "RfMessage":
    ...
    def is_completed(message: Message | None) -> bool:
        ...
        if isinstance(message, RfMessage):
            return not message.is_multipart or (
                message.is_multipart and message.is_multipart_completed
            )
        return True
