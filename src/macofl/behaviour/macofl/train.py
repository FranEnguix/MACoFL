...
#             unpickled_neighbour_weights = pickle.loads(
#                 codecs.decode(weights_and_losses[0].encode(), "base64")
#             )
#             unpickled_neighbour_losses = pickle.loads(
#                 codecs.decode(weights_and_losses[1].encode(), "base64")
#             )
...
#             unpickled_local_weights = pickle.loads(
#                 codecs.decode(self.agent.weights.encode(), "base64")
#             )
#             # print(unpickled_neighbour_weights[0]['layer_input.weight'])
#             # print(unpickled_local_weights)

#             # Apply consensus and update model
#             consensus_weights = self.agent.consensus.apply_consensus(
#                 unpickled_local_weights,
...
#                 consensus_weights[0], unpickled_neighbour_losses
#             )
#             self.agent.federated_learning.set_model()

#             # Update agent properties
#             self.agent.weights = codecs.encode(
#                 pickle.dumps(consensus_weights), "base64"
#             ).decode()
#             self.agent.losses = codecs.encode(
#                 pickle.dumps(unpickled_neighbour_losses), "base64"
#             ).decode()
#             print(
#                 colored(
#                     "[{}] Applied consensus after training and updated model".format(
#                         self.agent.name
#                     ),
#                     "red",
#                 )
...
#             local_weights,
#             local_losses,
#             self.agent.train_acc,
#             self.agent.train_loss,
#             self.agent.test_acc,
...
#         self.agent.weights = codecs.encode(
#             pickle.dumps(local_weights), "base64"
#         ).decode()
...
#         if (
#             Config.max_training_iterations >= 0
#             and self.agent.iterations > Config.max_training_iterations
#         ):
#             print(
#                 f"[{self.agent.name}] STOPPING. {self.agent.iterations} iterations reached."
#             )
#             await self.agent.stop_agent()
#         self.agent.iterations = self.agent.iterations + 1
#         print(
#             f"[{self.agent.name}] iteration number: {self.agent.iterations} / {Config.max_training_iterations}"
#         )
