...
class CommunicationState(State):
    ...
    async def on_start(self) -> None:
        ...
        self.agent.logger.debug(
            f"[{self.agent.current_round}] ({consensus_it_id}) Starting CommunicationState..."
        )
        ...
        try:
            ...
            if selected_neighbours:
                ...
                if self.agent.similarity_manager.function is not None:
                    all_responses_received = await self.similarity_vector_exchange(
    ...
    async def similarity_vector_exchange(self, neighbours: list[JID]) -> bool:
        ...
        if not vector:
            raise ValueError(
                "Trying to do the similarity vector phase without similarity function."
            )
        thread = str(uuid.uuid4())
        ...
        for neighbour in neighbours:
            await self.send_similarity_vector(
                thread=thread, vector=vector, neighbour=neighbour
            )
